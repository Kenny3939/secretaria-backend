...
cron.schedule('* * * * *', async () => {
  ...
  try {
    ...
    for (const cita of query24h.rows) {
      ...
      const fechaFmt = fechaObj.toLocaleString('es-GT', {
      ...
      await enviarMensajeWhatsApp(cita.phone_number, mensaje);
      await pool.query('UPDATE appointments SET reminder_24h_sent = true WHERE id = $1', [cita.id]);
    ...
    // ==========================================================
    const query2h = await pool.query(
      `
      SELECT 
        a.id, a.start_datetime, 
        s.name AS service_name, 
    ...
    for (const cita of query2h.rows) {
      ...
      await enviarMensajeWhatsApp(cita.phone_number, mensaje);
      await pool.query('UPDATE appointments SET reminder_2h_sent = true WHERE id = $1', [cita.id]);
    ...
    // ==========================================================
    const query15min = await pool.query(
      `
      SELECT 
        a.id, a.start_datetime, 
        c.name AS client_name, c.phone_number
    ...
    for (const cita of query15min.rows) {
      ...
      await enviarMensajeWhatsApp(cita.phone_number, mensaje);
      await pool.query('UPDATE appointments SET reminder_15min_sent = true WHERE id = $1', [cita.id]);
