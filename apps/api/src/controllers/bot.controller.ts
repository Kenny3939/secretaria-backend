...
import { enviarMensajeWhatsApp, enviarBotonesWhatsApp, enviarListaWhatsApp } from '../services/whatsapp.service';
import { procesarFechaHora, generarHorariosDisponibles } from '../utils/time.utils';
...
export const handleMessage = async (req: Request, res: Response) => {
  ...
  if (body.entry?.[0].changes?.[0].value.messages) {
    ...
    try {
      ...
      let clientQuery = await pool.query('SELECT id, name FROM clients WHERE phone_number = $1 AND business_id = $2', [
        numeroCliente,
        negocio.id,
      ]);
      ...
      if (clientQuery.rows.length > 0) {
        ...
        const newC = await pool.query('INSERT INTO clients (business_id, phone_number) VALUES ($1, $2) RETURNING id', [
          negocio.id,
          numeroCliente,
        ]);
        clienteId = newC.rows[0].id;
        nombreCliente = null;
        await pool.query('INSERT INTO conversations (business_id, client_id, current_step) VALUES ($1, $2, $3)', [
          negocio.id,
          clienteId,
          'welcome',
        ]);
      ...
      // =====================================================================
      const settingsQuery = await pool.query('SELECT off_hours_message, buffer_minutes FROM business_settings WHERE business_id = $1', [negocio.id]);
      ...
      if (fueraDeHorario && messageInfo.type === 'text') {
        ...
        if (debeAvisar) {
          ...
          await pool.query('UPDATE conversations SET last_off_hours_alert = NOW() WHERE client_id = $1', [clienteId]);
      ...
      if (entradaUsuario === 'opt_agendar') {
        ...
        const filas = srvs.map((s: any) => ({
          id: `srv_${s.id}`,
          title: s.name.substring(0, 24),
          description: `Q${s.price} | ${s.duration_minutes} min`,
        }));
        ...
        await pool.query("UPDATE conversations SET current_step = 'eligiendo_fecha', last_message = $1 WHERE client_id = $2", [
          JSON.stringify({ servicio_id: srvId, nombre_servicio: srv.name }),
          clienteId,
        ]);
        ...
        const hors = generarHorariosDisponibles(
          fechaBase,
        ...
        await pool.query("UPDATE conversations SET current_step = 'eligiendo_hora', last_message = $1 WHERE client_id = $2", [
          JSON.stringify(mem),
          clienteId,
        ]);
        ...
        if (!nombreCliente) {
          await pool.query("UPDATE conversations SET current_step = 'pidiendo_nombre', last_message = $1 WHERE client_id = $2", [
            JSON.stringify(mem),
            clienteId,
          ]);
          await enviarMensajeWhatsApp(numeroCliente, `¡Listo a las ${mem.hora}! 🎉 ¿Cuál es tu nombre?`);
        } else {
          await pool.query("UPDATE conversations SET current_step = 'confirmando_cita', last_message = $1 WHERE client_id = $2", [
            JSON.stringify(mem),
            clienteId,
          ]);
        ...
        await pool.query('INSERT INTO appointments (business_id, client_id, service_id, start_datetime, end_datetime, status) VALUES ($1, $2, $3, $4, $5, $6)', [
          negocio.id,
          clienteId,
          mem.servicio_id,
          ini,
      ...
      // =====================================================================
        ...
        cs.forEach(
          (c: any) =>
            (m += `• ${c.name}: ${new Date(c.start_datetime).toLocaleString('es-GT', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}\n`),
        );
        await pool.query("UPDATE conversations SET current_step = 'welcome' WHERE client_id = $1", [clienteId]);
        ...
        const filas = cs.map((c: any) => ({
          id: `${modo}_${c.id}`,
          title: c.name.substring(0, 24),
          description: new Date(c.start_datetime).toLocaleString('es-GT', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }),
        }));
        ...
        if (entradaUsuario.startsWith('can_')) {
          ...
          const citaCan = (await pool.query(`SELECT a.start_datetime, s.name FROM appointments a JOIN services s ON a.service_id = s.id WHERE a.id = $1`, [citaIdCan]))
            .rows[0];
          const fechaCan = citaCan
            ? new Date(citaCan.start_datetime).toLocaleString('es-GT', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
            : '';
          const msgNoti = `❌ ${nombreCliente || 'Un cliente'} canceló su cita de *${citaCan?.name || 'servicio'}* del ${fechaCan}.`;
          await pool.query(`INSERT INTO notifications (business_id, type, message, appointment_id) VALUES ($1, 'cancellation', $2, $3)`, [
            negocio.id,
            msgNoti,
            citaIdCan,
          ]);
          ...
          const cita = (await pool.query('SELECT a.service_id, s.name, s.duration_minutes FROM appointments a JOIN services s ON a.service_id = s.id WHERE a.id = $1', [citaId]))
            .rows[0];
          await pool.query("UPDATE conversations SET current_step = 'eligiendo_fecha_repro', last_message = $1 WHERE client_id = $2", [
            JSON.stringify({ cita_id: citaId, servicio_id: cita.service_id, nombre_servicio: cita.name, duracion: cita.duration_minutes }),
            clienteId,
          ]);
      ...
      // =====================================================================
        ...
        mem.fecha = entradaUsuario;
        mem.horarios_disponibles = hors;
        await pool.query("UPDATE conversations SET current_step = 'eligiendo_hora_repro', last_message = $1 WHERE client_id = $2", [
          JSON.stringify(mem),
          clienteId,
        ]);
      ...
      // =====================================================================
        ...
        await pool.query("UPDATE conversations SET current_step = 'welcome' WHERE client_id = $1", [clienteId]);
        await enviarBotonesWhatsApp(numeroCliente, t, [
          { id: 'opt_agendar', title: '📅 Agendar' },
          { id: 'btn_inicio', title: '⬅️ Menú' },
        ]);
      }
